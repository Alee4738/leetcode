...
import { ListNode, TreeNode } from './leetcodeTypes';
...
/*
Do DFS, maintain path
When you get to a leaf node (node.left === node.right === null), then ask if linked list at head is a subpath of path
This means we'll do the subpath calculation once for every leaf node, maximum |V| => O(V)

Think of more clever ways
While you're doing the DFS, crawl the linked list.
Always checking whether head.val === node.val. if it is, then do the DFS with head = head.next.
So, if head === null, then you know that you found a match
...
function helper(
  ...
  // console.log('helper curr', cur?.val, 'treeNode', treeNode?.val, 'head', head?.val)
  if (cur === null) {
    return true;
  }
  if (treeNode === null) {
  ...
  if (cur.val === treeNode.val) {
    if (
      helper(cur.next, treeNode.left, head) ||
      helper(cur.next, treeNode.right, head)
    ) {
      return true;
    }
  }
  // List broke (or never started), start looking from beginning
  return (
    helper(head, treeNode.left, head) || helper(head, treeNode.right, head)
  );
}
